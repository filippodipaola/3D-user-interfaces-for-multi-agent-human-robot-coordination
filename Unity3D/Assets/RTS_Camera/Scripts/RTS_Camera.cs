...
namespace RTS_Cam
{
    ...
    public class RTS_Camera : MonoBehaviour
    {
        ...
        private float prev_distanceToGround = 0f;
        ...
        private void HeightCalculation()
        {
            ...
            if(distanceToGround != targetHeight)
                ...
                {
                    difference = targetHeight - prev_distanceToGround;
                }
                else
                {
                    difference = targetHeight - distanceToGround;
                    prev_distanceToGround = distanceToGround;
                }
        ...
        private void Rotation()
        {
            ...
            {
                float move_index_y = -MouseAxis.y * Time.deltaTime * mouseRotationSpeed;
                m_Transform.Rotate(move_index_y, 0f, 0f);
                if (transform.localEulerAngles.x < 35f)
                {
                    transform.localEulerAngles = new Vector3(35f, transform.localEulerAngles.y, transform.localEulerAngles.z);
                }
                if (transform.localEulerAngles.z > 179f)
                {
                    transform.localEulerAngles = new Vector3(90f, transform.localEulerAngles.y, transform.localEulerAngles.z);
                }

                float move_index_x = -MouseAxis.x * Time.deltaTime * mouseRotationSpeed;
                // Code required for the camera to rotate around what the camera is effectively looking at!
                Ray ray = new Ray(transform.position, transform.eulerAngles);
                RaycastHit hit;
                Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, Mathf.Infinity, groundMask);
                transform.RotateAround(hit.point, Vector3.up, move_index_x);
        ...
        private float DistanceToGround()
        {
            ...
            // If the camera is above an area of the map where the 
            // there is no "ground", it returns 0, need to change it
            // to something else.
            return 0f;
        }
